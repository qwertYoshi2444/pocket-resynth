 ...
 * Twiddle factors pre-computed; bit-reversal table cached.
 */
class FFT {
  constructor(size) {
    ...
    // Pre-compute twiddle factors W_N^k = e^{-j2πk/N}
  ...
  /** Forward FFT (in-place). real, imag: Float64Array of length this.size */
  forward(real, imag) {
    const N = this.size;
    ...
    for (let s = 1; s <= this.log2Size; s++) {
      const m = 1 << s;
      const mh = m >> 1;
      const step = N / m; // stride into twiddle table
      for (let k = 0; k < N; k += m) {
        for (let j = 0; j < mh; j++) {
          const ti = j * step;
