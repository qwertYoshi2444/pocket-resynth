...
class SimpleZip {
  ...
  addFile(filename, uint8Array) {
    ...
    for (let i = 0; i < filename.length; i++) nameBytes[i] = filename.charCodeAt(i) & 0xFF;
    this.files.push({ name: nameBytes, data: uint8Array });
  }

  generate() {
    let localDataSize = 0, centralDirSize = 0;
    ...
    for (let i = 0; i < this.files.length; i++) {
      ...
      dv.setUint16(off, 10, true); off += 2; dv.setUint16(off, 10, true); off += 2;
      dv.setUint16(off, 0, true); off += 2; dv.setUint16(off, 0, true); off += 2;
      off += 4; // time/date
      ...
      dv.setUint16(off, 0, true); off += 2; dv.setUint16(off, 0, true); off += 2;
      dv.setUint16(off, 0, true); off += 2; dv.setUint32(off, 0, true); off += 4;
    ...
    dv.setUint32(off, off - cdOffset, true); off += 4;
